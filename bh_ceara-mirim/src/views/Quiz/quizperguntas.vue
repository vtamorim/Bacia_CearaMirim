<template>
  <div class="page-inner">
    ...
    <div v-if="!finished" class="quizperguntas card">
      <h2>{{ Lista[index] }}</h2>
      <p>
        Questão {{ index + 1 }}/<span class="total">{{ Lista.length }}</span>
      </p>
    ...
    <div v-else class="quizperguntas card resultado-final">
      ...
      <p>
        Você acertou <strong>{{ certas }}</strong> de {{ Lista.length }}
      </p>
...
const Alternativas = ref([
  ...
  [
    'Abastecimento urbano, indústria e mineração, irrigação e uso agropecuário, aquicultura e pesca, turismo e lazer.',
    'Uso apenas doméstico e recreativo, sem fins produtivos',
    'Somente navegação fluvial e transporte de cargas',
    'Apenas geração de energia elétrica e transporte hidroviário',
  ],
  [
    'Representações da sociedade civil, poder público e usuários de água',
    'Somente representantes do governo estadual',
    'Apenas organizações não governamentais ambientais',
    'Somente representantes de empresas privadas e indústrias',
  ],
]);
